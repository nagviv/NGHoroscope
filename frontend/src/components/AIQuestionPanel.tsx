  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/v1/ai/ask', {
        ...
        body: JSON.stringify({ birth_details: birthDetails, question, category: 'Career' })
      });
  ...
  return (
    <div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-6 space-y-4">
      <h3 className="text-lg font-bold font-cinzel text-amber-200 flex items-center gap-2">
        <Sparkles className="w-5 h-5 text-amber-400" /> AI Astrologer Q&A
      </h3>
      <form onSubmit={handleAsk} className="flex gap-2">
        ...
        />
        <button type="submit" disabled={loading} className="bg-amber-500 text-slate-950 font-semibold px-4 py-2 rounded-xl flex items-center gap-2">
      ...
      {response && (
        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 text-xs text-slate-300">
          <div className="font-bold text-amber-400">Analysis:</div>
