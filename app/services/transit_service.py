...
class TransitService:
    ...
    def calculate_transits(req: TransitRequest) -> TransitResponse:
        ...
        return TransitResponse(transit_date=target_dt.strftime("%Y-%m-%d"), transit_planets=transit["planets"], transits_from_lagna={p: ((d["sign_index"] - asc_idx) % 12) + 1 for p, d in transit["planets"].items()}, transits_from_moon={p: ((d["sign_index"] - moon_idx) % 12) + 1 for p, d in transit["planets"].items()}, sade_sati_status=check_sade_sati(moon_idx, transit["planets"]["Saturn"]["sign_index"]))
