...
def calculate_kp_chart(birth_dt: datetime, tz_offset: float, latitude: float, longitude: float) -> Dict[str, Any]:
    ...
    for i in range(1, 13):
        ...
        cusp_details.append({"cusp": i, "longitude": round(c_deg, 3), "degree_in_sign": round(c_deg % 30.0, 2), "sign": sub_info["sign"], "sign_lord": sub_info["sign_lord"], "star_lord": sub_info["star_lord"], "sub_lord": sub_info["sub_lord"]})
    planet_details = {}
    for p_name, pid in PLANET_IDS.items():
        ...
        planet_details[p_name] = {"longitude": round(p_lon, 3), "degree_in_sign": round(p_lon % 30.0, 2), "sign": sub_info["sign"], "sign_lord": sub_info["sign_lord"], "star_lord": sub_info["star_lord"], "sub_lord": sub_info["sub_lord"], "kp_house": 1}
    asc_sub = get_kp_sub_lord(ascmc[0] % 360.0)
    moon_sub = get_kp_sub_lord(planet_details["Moon"]["longitude"])
    return {
        "cusps": cusp_details, "planets": planet_details,
        "ruling_planets": {"lagna_sign_lord": asc_sub["sign_lord"], "lagna_star_lord": asc_sub["star_lord"], "moon_sign_lord": moon_sub["sign_lord"], "moon_star_lord": moon_sub["star_lord"], "day_lord": "Sun"}
    }
