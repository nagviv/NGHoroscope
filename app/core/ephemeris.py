...
def calculate_nakshatra(longitude: float) -> Dict[str, Any]:
    ...
    return {"name": NAKSHATRAS[nak_idx], "index": nak_idx, "pada": int(deg_in_nak // pada_span) + 1, "progress_degrees": deg_in_nak}
...
def compute_chart_raw(birth_dt: datetime, tz_offset: float, latitude: float, longitude: float, ayanamsa: int = swe.SIDM_LAHIRI) -> Dict[str, Any]:
    ...
    ascendant_data = {"longitude": asc_deg, "sign": RASHIS[asc_rashi_idx], "sign_index": asc_rashi_idx, "degree_in_sign": asc_deg % 30, "nakshatra": asc_nak["name"], "pada": asc_nak["pada"], "d9_sign": RASHIS[compute_d9_navamsha(asc_deg)], "d10_sign": RASHIS[compute_d10_dashamsha(asc_deg)]}
    planets_data = {}
    for name, pid in PLANET_IDS.items():
        ...
        planets_data[name] = {"longitude": lon, "sign": RASHIS[rashi_idx], "sign_index": rashi_idx, "degree_in_sign": lon % 30, "is_retrograde": speed < 0, "speed": speed, "house": ((rashi_idx - asc_rashi_idx) % 12) + 1, "nakshatra": nak["name"], "pada": nak["pada"], "d9_sign": RASHIS[compute_d9_navamsha(lon)], "d10_sign": RASHIS[compute_d10_dashamsha(lon)]}
    rahu_lon = planets_data["Rahu"]["longitude"]
    ketu_lon = (rahu_lon + 180.0) % 360.0
    ketu_rashi_idx = int(ketu_lon // 30)
    ketu_nak = calculate_nakshatra(ketu_lon)
    planets_data["Ketu"] = {"longitude": ketu_lon, "sign": RASHIS[ketu_rashi_idx], "sign_index": ketu_rashi_idx, "degree_in_sign": ketu_lon % 30, "is_retrograde": planets_data["Rahu"]["is_retrograde"], "speed": planets_data["Rahu"]["speed"], "house": ((ketu_rashi_idx - asc_rashi_idx) % 12) + 1, "nakshatra": ketu_nak["name"], "pada": ketu_nak["pada"], "d9_sign": RASHIS[compute_d9_navamsha(ketu_lon)], "d10_sign": RASHIS[compute_d10_dashamsha(ketu_lon)]}
