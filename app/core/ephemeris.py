...
def calculate_nakshatra(longitude: float) -> Dict[str, Any]:
    ...
    return {"name": NAKSHATRAS[nak_idx], "index": nak_idx, "pada": int((lon % nak_span) // (nak_span / 4.0)) + 1, "progress_degrees": lon % nak_span}
...
def compute_chart_raw(birth_dt: datetime, tz_offset: float, latitude: float, longitude: float, ayanamsa: int = swe.SIDM_LAHIRI) -> Dict[str, Any]:
    ...
    ascendant_data = {"longitude": asc_deg, "sign": RASHIS[asc_rashi_idx], "sign_index": asc_rashi_idx, "degree_in_sign": asc_deg % 30, "nakshatra": calculate_nakshatra(asc_deg)["name"], "pada": 1, "d9_sign": RASHIS[compute_d9_navamsha(asc_deg)], "d10_sign": RASHIS[compute_d10_dashamsha(asc_deg)]}
    planets_data = {}
    for name, pid in PLANET_IDS.items():
        res, _ = swe.calc_ut(jd_ut, pid, swe.FLG_SIDEREAL | swe.FLG_SPEED)
        lon = res[0] % 360.0
        rashi_idx = int(lon // 30)
        planets_data[name] = {"longitude": lon, "sign": RASHIS[rashi_idx], "sign_index": rashi_idx, "degree_in_sign": lon % 30, "is_retrograde": res[3] < 0, "speed": res[3], "house": ((rashi_idx - asc_rashi_idx) % 12) + 1, "nakshatra": calculate_nakshatra(lon)["name"], "pada": 1, "d9_sign": RASHIS[compute_d9_navamsha(lon)], "d10_sign": RASHIS[compute_d10_dashamsha(lon)]}
